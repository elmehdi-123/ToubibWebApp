...
export class PersonService {
  ...
  protected convertDateFromClient(person: IPerson): IPerson {
    const copy: IPerson = Object.assign({}, person, {
      dateDeNaissance: person.dateDeNaissance && person.dateDeNaissance.isValid() ? person.dateDeNaissance.format(DATE_FORMAT) : undefined
    });
    return copy;
  }
  ...
  protected convertDateArrayFromServer(res: EntityArrayResponseType): EntityArrayResponseType {
    if (res.body) {
      res.body.forEach((person: IPerson) => {
        person.dateDeNaissance = person.dateDeNaissance ? moment(person.dateDeNaissance) : undefined;
      });
    }
    return res;
  }
